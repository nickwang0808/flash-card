import { and, arrayContains, asc, desc, eq, ilike, inArray, lt, or, type SQL } from 'drizzle-orm';
import { z } from 'zod';
...
async function cardAggregate(db: Parameters<typeof cardsOwnedBy>[0], userId: string, cardId: string, deckId: string): Promise<Card> {
  const card = await requireDeckForCard(db, userId, cardId, deckId);
  const cadenceRows = await cardCadencesOwnedBy(db, userId).where(eq(cardCadences.cardId, cardId)).orderBy(asc(cardCadences.direction));
  return mapCard(card, cadenceRows.map(({ cadence }) => mapCadence(cadence)));
}
async function aggregatesFor(db: Parameters<typeof cardsOwnedBy>[0], userId: string, rows: readonly (typeof cards.$inferSelect)[]): Promise<Card[]> {
  if (rows.length === 0) return [];
  const cadenceRows = await cardCadencesOwnedBy(db, userId).where(inArray(cardCadences.cardId, rows.map((row) => row.id))).orderBy(asc(cardCadences.direction));
  const byCardId = new Map<string, CardCadence[]>();
  for (const { cadence } of cadenceRows) {
    const values = byCardId.get(cadence.cardId) ?? [];
    values.push(mapCadence(cadence));
    byCardId.set(cadence.cardId, values);
  }
  return rows.map((row) => mapCard(row, byCardId.get(row.id)));
}
...
export const cardRouter = t.router({
  ...
  search: protectedProcedure.input(CardSearchInputSchema).output(z.object({ cards: z.array(CardSchema), pageInfo: PageInfoSchema })).query(async ({ ctx, input }) => {
    ...
    const values = rows.map(({ cards: card }) => card);
    const pageRows = values.length > input.pagination.limit ? values.slice(0, input.pagination.limit) : values;
    return { cards: await aggregatesFor(ctx.db, ctx.identity.userId, pageRows), pageInfo: { nextCursor: values.length > input.pagination.limit ? encodeCursor(pageRows.at(-1)!.createdAt, pageRows.at(-1)!.id) : null } };
  }),
  create: protectedProcedure.input(CardCreateInputSchema).output(CardSchema).mutation(async ({ ctx, input }) => {
    ...
    return ctx.db.transaction(async (tx) => {
      ...
      const directions = input.reversible ? ['forward', 'reverse'] as const : ['forward'] as const;
      const cadenceRows = await tx.insert(cardCadences).values(directions.map((direction) => ({ cardId: row.id, direction, createdAt: timestamp, updatedAt: timestamp }))).returning();
  ...
  update: protectedProcedure.input(CardUpdateInputSchema).output(CardSchema).mutation(async ({ ctx, input }) => {
    ...
    return ctx.db.transaction(async (tx) => {
      ...
      if (before.reversible !== input.reversible) throw new ApplicationError('INVALID_STATE', 'Card direction cannot change after creation');
      ...
      const card = mapCard(rows[0], before.cadences);
  ...
  restore: protectedProcedure.input(CardSuspendInputSchema).output(z.object({ card: CardSchema, queue: ReplacementQueueSchema.shape.queue })).mutation(async ({ ctx, input }) => {
    ...
    if (rows.length === 0) throw await versionError(ctx.db, ctx.identity.userId, input.cardId, input.deckId, input.expectedVersion);
    return { card: await cardAggregate(ctx.db, ctx.identity.userId, input.cardId, input.deckId), queue: await queueSnapshot(ctx.db, ctx.identity.userId, input.deckId, input.queue, ctx.now) };
  }),
  ...
  revisions: protectedProcedure.input(CardRevisionsInputSchema).output(z.object({ revisions: z.array(CardRevisionSchema), pageInfo: PageInfoSchema })).query(async ({ ctx, input }) => {
    ...
    if (after) conditions.push(or(lt(cardRevisions.createdAt, after.timestamp), and(eq(cardRevisions.createdAt, after.timestamp), lt(cardRevisions.id, after.id))));
    ...
    const values = rows.map(({ revision }) => ({ ...revision, createdAt: toIsoTimestamp(revision.createdAt) } as CardRevision));
    const pageItems = values.length > input.pagination.limit ? values.slice(0, input.pagination.limit) : values;
    return { revisions: pageItems, pageInfo: { nextCursor: values.length > input.pagination.limit ? encodeCursor(pageItems.at(-1)!.createdAt, pageItems.at(-1)!.id) : null } };
  }),
