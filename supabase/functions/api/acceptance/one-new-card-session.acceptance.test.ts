...
describe('one new card study session', () => {
  test('rates, reloads history, reaches due boundary, and restores exact state on undo', async () => {
    ...
    expect(restored).toMatchObject({ nextReviewAt: null, intervalDays: null, reviewCount: 0, lapseCount: 0, version: 2, updatedAt: expected.nextReviewAt });
    ...
    expect({ ...storedCard, updatedAt: toIsoTimestamp(storedCard.updatedAt) }).toMatchObject({ version: 2, nextReviewAt: null, updatedAt: expected.nextReviewAt });
  });
});
