...
describe('request-scoped application clock over the served Edge Function', () => {
  test('keeps concurrent users independent and persists one request instant', async () => {
    ...
    const history = await a.api.review.history({ cardId: cardA.id, deckId: deckA.id, pagination: { limit: 10 } });
    ...
    expect(toIsoTimestamp(updatedCard.updatedAt)).toBe(a.clock.iso());
    expect(updatedCard.nextReviewAt === null ? null : toIsoTimestamp(updatedCard.nextReviewAt)).toBe(expectedState.nextReviewAt);
  });
