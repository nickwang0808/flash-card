...
describe('review persistence and history', () => {
  test('makes concurrent idempotent requests one logical review and rejects competing requests', async () => {
    ...
    const replay = await Promise.all([
      actor.api.review.rate({ cadenceId: same.cadences[0].id, deckId: deck.id, rating: 'good', expectedVersion: 0, requestId, queue }),
      actor.api.review.rate({ cadenceId: same.cadences[0].id, deckId: deck.id, rating: 'again', expectedVersion: 0, requestId, queue }),
    ]);
    ...
    const raced = await Promise.allSettled([
      actor.api.review.rate({ cadenceId: different.cadences[0].id, deckId: deck.id, rating: 'good', expectedVersion: 0, requestId: crypto.randomUUID(), queue }),
      actor.api.review.rate({ cadenceId: different.cadences[0].id, deckId: deck.id, rating: 'again', expectedVersion: 0, requestId: crypto.randomUUID(), queue }),
    ]);
  ...
  test('guards undo order, retains events, and keyset-paginates history', async () => {
    ...
    const first = await actor.api.review.rate({ cadenceId: card.cadences[0].id, deckId: deck.id, rating: 'good', expectedVersion: 0, requestId: crypto.randomUUID(), queue });
    actor.clock.advance({ minutes: 1 });
    const second = await actor.api.review.rate({ cadenceId: card.cadences[0].id, deckId: deck.id, rating: 'good', expectedVersion: 1, requestId: crypto.randomUUID(), queue });
    ...
    const history = await actor.api.review.history({ cadenceId: card.cadences[0].id, deckId: deck.id, pagination: { limit: 1 } });
    const later = await actor.api.review.history({ cadenceId: card.cadences[0].id, deckId: deck.id, pagination: { limit: 1, cursor: history.pageInfo.nextCursor } });
    ...
    await expect(actor.api.review.history({ cadenceId: card.cadences[0].id, deckId: deck.id, pagination: { limit: 1, cursor: 'not-a-cursor' } })).rejects.toThrow('Invalid pagination cursor');
  });
});
