...
import type { CardData, CardRepository } from '../../src/services/card-repository';
import { SupabaseStorageService } from '../../src/services/supabase-storage';

// Mock Supabase client with authenticated user
vi.mock('../../src/services/supabase', () => ({
  supabase: {
    auth: { getUser: vi.fn(() => Promise.resolve({ data: { user: { id: 'test-user-id' } }, error: null })) },
    from: vi.fn(() => {
      const chain: any = {};
...
import { setStorageService, runSync, notifyChange } from '../../src/services/replication';
...
describe('Replication with SupabaseStorageService', () => {
  ...
  it('notifyChange pushes card immediately', async () => {
    const card = {
      id: 'deck|hello',
      deckName: 'deck',
      term: 'hello',
