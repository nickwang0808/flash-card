...
export const cards = pgTable(
  'cards',
  {
    ...
    schedulerVersion: integer('scheduler_version'),
    version: integer('version').notNull().default(0),
  },
  (table) => [
    ...
    check('cards_cadence_phase_check', sql`${table.cadencePhase} in ('learning', 'review')`),
    ...
    check('cards_scheduler_version_check', sql`${table.schedulerVersion} is null or ${table.schedulerVersion} > 0`),
    check('cadence_fields_all_or_none', sql`(${table.cadencePhase} is null and ${table.nextReviewAt} is null and ${table.intervalDays} is null and ${table.schedulerVersion} is null) or (${table.cadencePhase} is not null and ${table.nextReviewAt} is not null and ${table.intervalDays} is not null and ${table.schedulerVersion} is not null)`),
