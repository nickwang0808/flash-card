...
export const cards = pgTable(
  ...
  (table) => [
    ...
    unique('cards_import_request_id_key').on(table.importRequestId),
    check('cards_import_fields_all_or_none', sql`(${table.importRequestId} is null and ${table.importPayloadHash} is null) or (${table.importRequestId} is not null and ${table.importPayloadHash} is not null)`),
...
export const reviewEvents = pgTable(
  'review_events',
  {
    ...
    origin: text('origin', { enum: ['native', 'imported'] }).notNull().default('native'),
  ...
  (table) => [
    ...
    unique('review_events_cadence_id_sequence_key').on(table.cadenceId, table.sequence),
    index('review_events_cadence_sequence_idx').on(table.cadenceId, table.sequence.desc()),
    check('review_events_origin_check', sql`${table.origin} in ('native', 'imported')`),
    check('review_events_rating_check', sql`${table.rating} is null or ${table.rating} in ('again', 'hard', 'good', 'easy')`),
    check('review_events_result_check', sql`(${table.origin} = 'native' and ${table.rating} is not null) or (${table.origin} = 'imported' and (${table.rating} is not null or ${table.recalled} is not null))`),
    check('review_events_duration_ms_check', sql`${table.durationMs} is null or ${table.durationMs} >= 0`),
    check('review_events_sequence_check', sql`${table.sequence} > 0`),
